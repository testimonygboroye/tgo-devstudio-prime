...
export default async function StackItemsListPage() {
  const session = await getServerSession();
  guardCanView(session!, "stackItems");
  await connectToDatabase();
  const items = await StackItem.find().sort({ category: 1, displayOrder: 1 }).lean();
    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold brand-gradient-text">Stack Items</h1>
        </div>
        <Link
          href={`${basePath}/stack-items/new`}
      ...
      <div className="mt-8 space-y-3">
        {items.length === 0 && <p className="text-neutral-400">No stack items yet. Add your first one.</p>}
        {items.map((item) => (
          <Link
            key={item._id.toString()}
            href={`${basePath}/stack-items/${item._id.toString()}`}
