    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold brand-gradient-text">Services</h1>
        </div>
        <Link
      ...
      <div className="mt-8 space-y-3">
        {services.length === 0 && <p className="text-neutral-400">No services yet. Add your first one.</p>}
        {services.map((service) => (
          <Link
            key={service._id.toString()}
            href={`${basePath}/services/${service._id.toString()}`}
            className="block rounded-lg border border-base-800 bg-base-900 p-4 hover:border-brand-cyan-400"
          >
            <div className="flex items-center justify-between">
              <span className="font-semibold text-neutral-100">{service.title}</span>
              <span
            ...
            <p className="mt-1 text-sm text-neutral-400">{service.summary}</p>
          </Link>
        ))}
      </div>
    </div>
